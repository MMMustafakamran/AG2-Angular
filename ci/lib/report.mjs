 ...
 *
 * It is not the same document as `autorecorder/videos/MANIFEST.md`, which the
 * consolidate job writes. This one describes THIS RUN — what resolved, what
 * came up, what was captured. The manifest describes the CLIPS ON DISK and
 * whether the source they show has moved since. Neither answers the other's
 * question.
 */
 ...
 * a run against @copilotkit/runtime 1.69.3 reported "^1.69.0". That made the
 * report misleading about the one thing the run exists to discover.
 *
 ...
 * (`ag2[ag-ui,openai]>=1.0.3,<2`) while ci/automate.mjs runs `uv sync
 * --upgrade`, which resolves past them. Reporting the specifier therefore
 * named a version the run had deliberately moved off -- and named it "Version".
 *
...
function backendVersions(dir) {
  ...
  // optional group after the name drops extras -- "ag2[ag-ui,openai]" is
  // locked under plain "ag2".
...
export function generateReport(data) {
  ...
  lines.push(`- **Dependencies:** \`${report.refreshedDeps ? 'Re-resolved (ranges)' : 'From lockfile'}\`\n`);
  ...
  lines.push(
    `- **AG2 backend (\`:${BACKEND_PORT}/health\`):** ${
  ...
  // What "recorded" means here, said in the artifact rather than only in
  // readme.md: most of these clips exist to demonstrate a defect and write the
  // finding on screen. A green run means each clip captured what it was meant
  // to capture, not that every feature works.
  lines.push(
    '> A clip counts as recorded when it captured what it was meant to capture.',
  );
  lines.push(
    '> Several of these pages are recorded to demonstrate a defect — the empty',
  );
  lines.push(
    '> A2UI surface, the dropped shared-state write, the interrupt panels that',
  );
  lines.push('> never fire — and each writes its finding on screen. See readme.md.');
  lines.push('');
